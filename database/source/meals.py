...
def get_content(html):
    ...
    items = soup.find_all('div', class_='subcategory-title')
    ...
    for item in items:
        ...
        category_title = item.find('h4', class_='_1oqru0l5').get_text()
        if(category_title == 'Популярное' or category_title == rest_title):
            continue

        meals = item.find_all('div', class_='x8a5w00')
        for meal in meals:
            meal_title = meal.find('span', class_='_15ztpd65').get_text()
            print(meal_title)
            try:
                meal_image = HOST + meal.find('img', class_='_15ztpd62').get('srcset').split()[0]
            except:
                meal_image = ''
            meal_price = meal.find_all('span', class_='_13un09z0')[1].get_text(strip=True).replace(' ', '').replace('₽', '')
            meal_price = unicodedata.normalize("NFKD", meal_price).replace(" ", "")
            try:
                meal_weight = meal.find('span', class_='_1mac6u61').get_text()
            except:
                meal_weight = ''
            restaurants.append({
...
def save_file(items, path):
    with open(path, 'a', newline='', encoding='utf-16') as file:
        ...
        for item in items:
            ...
            writer.writerow([rest_title, item['category_title'], item['meal_title'], item['meal_image'], item['meal_price'], item['meal_weight']])
...
def parse():
    ...
    if os.path.exists(file_path):
        ...
        with open(file_path, 'r') as f:
            ...
            save_file(restaurants, 'meals.csv')
            print('Success!')
    else:
        print('Error')
