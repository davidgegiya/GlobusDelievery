...
URL = 'https://www.delivery-club.ru/moscow/page6'
...
def get_content(html):
    ...
    try:
        while True:
            link = soup.find('a', class_='vendor-list__btn--load-more').get('href')
            print(HOST + link)
    ...
    for item in items:
        try:
            rest_img = item.find('span', class_='vendor-item__cover').get('data-src')
        except:
            rest_img = ""
        try:
            title = item.find('h3', class_='vendor-item__title-text').get_text()
        except:
            title = ""
        try:
            delivery_time = item.find('span', class_='vendor-item__delivery-time').get_text(strip = True)
        except:
            delivery_time = 'неизвестно'
        try:
            delivery_price = item.find('span', class_='vendor-item__info-item').get_text()
        except:
...
def save_file(items,path):
    with open(path, 'a', newline='', encoding='utf-16') as file:
        ...
        for item in items:
            ...
            writer.writerow([item['rest_title'], item['image'],item['delivery_time'], item['delivery_price'], item['rating']])
